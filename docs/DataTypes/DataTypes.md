...
C++ requires the users to specify the specific data type of each variable before it is used. The primary C++ built-in atomic data types are:
- `int` (integer)
- `float` (floating point)
- `double` (double precision floating point)
- `bool` (Boolean)
- `char` (character)
- `pointer` (special type that holds a memory location)
...
In Python, `//` is used for integer division. In C++, dividing two integers returns the integer portion of the quotient, removing the fractional part. To obtain the whole quotient, one of the numbers must be declared as a `float`.

### Mathematical Operations
- Exponentiation in C++ is done using `pow()` from the `cmath` library.
- The remainder (modulo) operator is `%`.
...
int main(){
    ...
    cout << float(6)/3 << endl;
    cout << float(7)/3 << endl;
    cout << 7/3 << endl; // Integer division in C++
    cout << 7%3 << endl;
    cout << float(3)/6 << endl;
    cout << 3/6 << endl;
    cout << 3%6 << endl;
...
When declaring numeric variables in C++, modifiers like `short`, `long`, and `unsigned` can optionally be used to optimize memory usage.

### Quiz: Integer Division in C++
> What is the result of `3/2` in C++?
> 
> - [x] `1` (Correct! This is like `3//2` in Python.)
> - [ ] `1.5` (Incorrect. Integer division is used.)
> - [ ] `2` (Incorrect. Integer division is used.)
> - [ ] A run-time error will occur. (Incorrect. C++ will execute the operation.)
> - [ ] None of the above (Incorrect.)

### Quiz: Exponentiation in C++
> How do I raise 4 to the 5th power in C++?
> 
> - [ ] `4**5` (Incorrect. `**` is used in Python, not C++.)
> - [ ] `5**4` (Incorrect. `**` is not valid in C++.)
> - [ ] `4^5` (Incorrect. `^` is a bitwise XOR operator in C++.)
> - [x] `pow(4, 5)` (Correct! Remember to include the `cmath` library.)

---

## Boolean Data

Boolean data types are named after **George Boole**, an English mathematician. However, in C++, the keyword `bool` is not capitalized. The possible values are `true` and `false` (lowercase). Be sure to note the difference from Python, where `True` and `False` are capitalized.

### Boolean Operators in C++ vs. Python
...
The internally stored values for `true` and `false` are `1` and `0`, respectively. This is reflected in output as well.
...
### Example: Basic Relational and Logical Operators
#### C++
```cpp
#include <iostream>
using namespace std;
...
When a C++ variable is declared, memory space is allocated to store its value. A variable can be initialized at declaration.

### Example: Boolean Pitfall in C++
```cpp
#include <iostream>
using namespace std;
...
int main(){
    ...
    theBool = 4; // Assigning an integer to a boolean
    cout << theBool << endl;
    
    return 0;
}
---
The declaration `int theSum = 4;` creates a variable called `theSum` and initializes it to hold the value `4`. As in Python, the right-hand side of each assignment statement is evaluated, and the resulting value is "assigned" to the variable named on the left-hand side. Here, the type of the variable is `int` (integer). 

In Python, which is dynamically typed, if the type of the data changes in the program, the type of the variable also changes. However, in C++, the data type of a variable is fixed at the time of declaration. This is a feature of C++'s' static typing. In C++, a variable can hold only one type of data throughout its lifetime. 

A common pitfall in C++ is that the compiler will often try to perform the assignment without raising an error, even if the assignment seems incorrect. In the following example, notice what happens in the output.
...
int main() {
    int theSum = 4;
    theSum = 5.5; // Assigning a float to an integer variable
    cout << theSum << endl; // Output: 5
    return 0;
}
--- -->
...
To view a copy of this license, visit [Creative Commons License](http://creativecommons.org/licenses/by-nc-sa/4.0/).  
